...
const Navbar = () => {
  return (
    <div className='top-0  mt-5 pb-4  left-0 w-full fixed z-99 rounded-full backdrop-blur-lg border overflow-hidden border-white' style={{height:'15vh'}}  >
        ...
        {/* mobile menu */}
        <div className={`md:hidden ${showmm ? 'fixed w-1/2' : ' w-0 overflow'} fixed top-0 right-0 bottom-0 w-full overflow-hidden bg-white transition-all`}>
             
             
             <ul className='flex flex-col gap-2 mt-5 px-6 text-lg  font-medium items-center'>
             <a onClick={()=>setShowmm(false)} href="#Header" className='px-4 py-2 rounded-full inline-block'>Home</a>
             <a onClick={()=>setShowmm(false)} href="#About" className='px-4 py-2 rounded-full inline-block'>About</a>
             <a onClick={()=>setShowmm(false)} href="#Projects" className='px-4 py-2 rounded-full inline-block'>Projects</a>
             <a onClick={()=>setShowmm(false)} href="#Testimonials" className='px-4 py-2 rounded-full inline-block'>Testimonials</a>
